...
const features = [
  {
    ...
    description: "Фильтры, рекомендации и релевантность как на hh.ru — только с AI-ранжированием под ваш профиль.",
  },
  {
    icon: Bot,
    title: "AI-агент 24/7",
...
const plans = [
  { name: "Free", price: "0", desc: "Базовый поиск и отклики" },
  { name: "Premium", price: "29 000", desc: "AI для соискателей", highlight: true },
  { name: "Business", price: "99 000", desc: "AI для работодателей" },
];
    ...
    <section className="relative overflow-hidden px-4 pb-20 pt-16 md:px-6 md:pt-24">
      ...
      <div className="relative mx-auto max-w-6xl">
        <div className="animate-fade-in-up mx-auto max-w-3xl text-center">
          ...
          <h1 className="mt-6 text-4xl font-bold leading-[1.1] tracking-tight md:text-6xl">
            ...
            <span className="inline-flex items-center gap-1">
              <span className="rounded-lg bg-[var(--hh-red)] px-2 py-0.5 text-white">hh</span>
            </span>
            , но с{" "}
            <span className="text-gradient">AI-агентом</span> и подпиской
          </h1>
          <p className="mx-auto mt-6 max-w-2xl text-lg leading-relaxed text-muted-foreground">
            TalentHub AI объединяет вакансии, отклики и умного помощника для соискателей и работодателей.
            Premium — для карьеры, Business — для найма.
          </p>
        </div>

        <div className="animate-fade-in-up delay-200 mx-auto mt-10 flex max-w-2xl flex-col gap-3 sm:flex-row sm:items-center">
          <div className="relative flex-1">
            <Search className="absolute left-4 top-1/2 size-5 -translate-y-1/2 text-muted-foreground" />
            <input
              type="text"
              placeholder="Должность, компания или навык"
              className="h-14 w-full rounded-xl border border-border bg-white pl-12 pr-4 text-base shadow-sm transition-shadow focus:border-[var(--brand)] focus:outline-none focus:ring-4 focus:ring-[var(--brand)]/15"
            />
          </div>
          <Button variant="shine" size="xl" className="shrink-0 sm:min-w-[160px]" asChild>
            <Link href="/auth/register">Найти вакансии</Link>
          </Button>
        </div>
        ...
        <div className="animate-scale-in delay-400 relative mx-auto mt-16 max-w-4xl">
          <div className="overflow-hidden rounded-2xl border border-border bg-white shadow-2xl shadow-[var(--brand)]/10">
          ...
          <div className="animate-float absolute -bottom-4 -right-2 hidden rounded-2xl border border-border bg-white p-4 shadow-xl md:block">
...
function MockJobCard({
  ...
  return (
    <article className="rounded-xl border border-border p-4 transition-all duration-300 hover:border-[var(--brand)]/40 hover:shadow-md">
      ...
      <Button variant="brand" size="sm" className="mt-4 w-full">
        Откликнуться
      </Button>
    </article>
  );
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {["Показать вакансии", "Улучшить резюме", "Советы по интервью"].map((chip) => (
                  <button
                    key={chip}
                    type="button"
                    className="rounded-full border border-white/20 px-3 py-1.5 text-xs transition-colors hover:bg-white/15"
                  >
                    {chip}
                  </button>
                ))}
    ...
    <section id="pricing" className="px-4 py-20 md:px-6 md:py-28">
      <div className="mx-auto max-w-6xl">
        <div className="text-center">
          ...
          <p className="mt-4 text-lg text-muted-foreground">Месячная подписка. Скидка 20% при оплате за год.</p>
        </div>
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {plans.map((plan, i) => (
            <article
              key={plan.name}
              className={cn(
                "relative rounded-2xl border bg-white p-6 transition-all duration-300 hover:-translate-y-1 animate-fade-in-up",
                plan.highlight
            ...
            >
              ...
              <p className="mt-6">
                ...
                {plan.price !== "0" && <span className="text-muted-foreground"> so&apos;m/мес</span>}
              </p>
              <Button
                ...
                asChild
              >
                <Link href="/candidate/dashboard">{plan.highlight ? "Подключить" : "Подробнее"}</Link>
              </Button>
            </article>
          ))}
        </div>
